...
export class PedidoArticuloService extends ApiService {
    ...
    addArticulo(pedidoId: number, articulo: CreatePedidoArticuloDto): Observable<{ data: PedidoArticulo }> {
        return this.post<{ data: PedidoArticulo }>(`${this.getBaseUrl()}/${pedidoId}/articulos`, articulo);
    }

    /**
     * Actualizar un artículo de un pedido
     */
    updateArticulo(pedidoId: number, articuloId: number, cambios: UpdatePedidoArticuloDto): Observable<{ data: PedidoArticulo }> {
        return this.put<{ data: PedidoArticulo }>(`${this.getBaseUrl()}/${pedidoId}/articulos/${articuloId}`, cambios);
    }
    ...
    deleteArticulo(pedidoId: number, articuloId: number): Observable<void> {
        return this.delete<void>(`${this.getBaseUrl()}/${pedidoId}/articulos/${articuloId}`);
    }
}
