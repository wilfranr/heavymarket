...
export const selectUserPermissions = createSelector(selectCurrentUser, (user) => user?.permissions || []);

export const selectUserName = createSelector(selectCurrentUser, (user) => user?.name || '');
...
export const selectHasAnyRole = (requiredRoles: string[]) => createSelector(selectUserRoles, (roles) => requiredRoles.some((role) => roles.includes(role)));
