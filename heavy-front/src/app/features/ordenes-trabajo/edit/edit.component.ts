      ...
      @if (loading()) {
        ...
        <form [formGroup]="ordenTrabajoForm" (ngSubmit)="onSubmit()">
          ...
          <div class="flex justify-content-end gap-2 mt-4">
            ...
            <p-button
              label="Actualizar Orden de Trabajo"
              icon="pi pi-check"
              type="submit"
              [loading]="saving()"
              [disabled]="ordenTrabajoForm.invalid">
            </p-button>
          </div>
        </form>
      }
...
export class EditComponent implements OnInit {
  ...
  private initForm(ordenTrabajo: any): void {
    this.ordenTrabajoForm = this.fb.group({
      ...
      fecha_ingreso: [
        ordenTrabajo.fecha_ingreso ? new Date(ordenTrabajo.fecha_ingreso).toISOString().split('T')[0] : null,
      ],
      fecha_entrega: [
        ordenTrabajo.fecha_entrega ? new Date(ordenTrabajo.fecha_entrega).toISOString().split('T')[0] : null,
      ],
  ...
  onSubmit(): void {
    ...
    const data: UpdateOrdenTrabajoDto = {
      ...
      fecha_ingreso: formValue.fecha_ingreso
        ? new Date(formValue.fecha_ingreso).toISOString().split('T')[0]
        : undefined,
      fecha_entrega: formValue.fecha_entrega
        ? new Date(formValue.fecha_entrega).toISOString().split('T')[0]
        : undefined,
